...
inline SQLParam MakeSQLParam(int val) { return SQLParam{val}; }

inline SQLParam MakeSQLParam(const std::string &val) { return SQLParam{val}; }

inline SQLParam MakeSQLParam(const std::vector<uint8_t> &val) {
  return SQLParam{val};
}

inline SQLParam MakeSQLParam(bool val) { return SQLParam{val}; }

inline SQLParam MakeSQLParam(double val) { return SQLParam{val}; }
...
inline std::string FormatSQL(const std::string& query, const std::vector<std::string>& params) {
    ...
    for (const auto& p : params) sqlParams.push_back(SQLParam{p});
